...
--------------------------------------------------------------------------------------\n
"""
function allPlots(n::NLOpt,r::Result,s::Settings,eval::Int64)
  stp = [statePlot(n,r,s,eval,st) for st in 1:n.numStates];
  ctp = [controlPlot(n,r,s,eval,ctr) for ctr in 1:n.numControls];
  all = [stp;ctp];
...
function statePlot(n::NLOpt,r::Result,s::Settings,eval::Int64,st::Int64)
  # plot the limits
  stp=plot(0,leg=:false)
  t_vec = linspace(r.dfs[eval][:t][1],r.dfs[eval][:t][end],s.L);
  ...
  # plot the values
  plot!(r.dfs[eval][:t],r.dfs[eval][n.state.name[st]],w=s.lw2,leg=:false);
...
function statePlot(n::NLOpt,r::Result,s::Settings,eval::Int64,st1::Int64,st2::Int64)
  # plot the limits
  stp=plot(0,leg=:false)
  ...
  # plot the values
  plot!(r.dfs[eval][n.state.name[st1]],r.dfs[eval][n.state.name[st2]],w=s.lw1,leg=:false);
  adjust_axis(xlims(),ylims());
...
function controlPlot(n::NLOpt,r::Result,s::Settings,eval::Int64,ctr::Int64)
  # plot the limits
  ...
  # plot the values
  plot!(r.dfs[eval][:t],r.dfs[eval][n.control.name[ctr]],w=s.lw2,leg=:false)
  adjust_axis(xlims(),ylims());
